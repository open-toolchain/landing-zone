...
locals {
  base_resource_group_contains_4_groups = regex("4", tostring(length(module.resource_groups_base.value)))
  base_rg_names                         = module.resource_groups_base.value.*.name
  base_rg_contains_Default              = regex("true", tostring(contains(local.base_rg_names, "Default")))
  base_rg_does_not_create_Default       = regex("false", tostring(module.resource_groups_base.value[0].create))
...
locals {
  ...
  all_rg_does_not_create_Default       = regex("false", tostring(module.resource_groups_all.value[0].create))
