...
locals {
  ...
  subnet_map = {
    # For each zone
    for zone in ["zone-1", "zone-2", "zone-3"]:
    # Return a list containing all subnets in that zone with the id of the acl specified in each acl name
    zone => [
      for subnet in var.subnets[zone]:
      merge(subnet, {
        acl_id = ibm_is_network_acl.network_acl[subnet.acl_name].id
      })
    ]
  }
}
